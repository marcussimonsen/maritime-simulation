...
class Plotting(object):
    def __init__(self, param, defaults):
        list_defaults = ['Lx', 'Ly', 'cax', 'figwidth', 
                         'typewave', 'vectorscale']
        list_param = ['varplot', 'macuser']
        for k in list_defaults:
            self.__dict__[k] = defaults[k]
...
def plotvar(param, defaults, field2d, varname, cax=None):
    param.varplot = varname
    if cax is None:
        maxi = np.max(np.abs(field2d.ravel()))
        defaults['cax'] = [-maxi, maxi]
    else:
        defaults['cax'] = cax

    fig = Plotting(param, defaults)
